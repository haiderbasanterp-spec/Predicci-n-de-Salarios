...
st.set_page_config(
    page_title="Predictor de Salario",
    page_icon="💼",
    layout="centered",
...
@st.cache_resource
def cargar_modelo():
    """Carga el modelo y los artefactos de preprocesamiento."""
    archivos = {
...
# Estas listas deben coincidir con los valores usados en el entrenamiento.
# Si tienes los encoders cargados, se usan las clases del encoder.
# Si no, se usan estas listas como fallback.
...
# Si los encoders están cargados, usar sus clases reales
...
def predecir_salario(experience, company_size, latitude, longitude,
    ...
    """Preprocesa los inputs y retorna la predicción del modelo."""
    ...
    def encode(col_name, valor, fallback_list):
        if encoders and col_name in encoders:
            ...
            if valor in le.classes_:
                ...
                # Si el valor no está, usar el índice medio
    ...
    # Armar vector de features en el mismo orden del entrenamiento:
    # ['Experience_num', 'Company Size', 'latitude', 'longitude',
    #  'year', 'Work Type', 'Qualifications', 'Country', 'Role']
    X_input = np.array([[
...
st.markdown('<p class="hero-sub">Ingresa las características del empleo y predice el salario promedio</p>', unsafe_allow_html=True)

# Aviso si falta el modelo
if errores:
    st.markdown(f"""
    <div class="warning-box">
        ⚠️ <strong>Modelo no encontrado.</strong><br>
        Asegúrate de que los archivos <code>modelo_salario.keras</code>, 
        <code>scaler_salario.pkl</code> y <code>encoders_salario.pkl</code> 
        estén en la misma carpeta que este archivo <code>app.py</code>.<br><br>
        <em>Detalle: {', '.join(errores)}</em>
    </div>
    """, unsafe_allow_html=True)

# ── Tarjeta 1: Experiencia y empresa ─────────────────────────────────────────
...
with col1:
    experience = st.slider(
        ...
        help="Número de años de experiencia requeridos para el cargo"
    )
with col2:
    company_size = st.number_input(
        "Tamaño de la empresa (empleados)",
        min_value=10, max_value=500000, value=5000, step=500,
        help="Número aproximado de empleados en la empresa"
    )
...
with col7:
    latitude = st.number_input(
        "Latitud",
        min_value=-90.0, max_value=90.0, value=4.71,
        format="%.4f",
        help="Coordenada de latitud de la ubicación del empleo"
    )
with col8:
    longitude = st.number_input(
        "Longitud",
        min_value=-180.0, max_value=180.0, value=-74.07,
        format="%.4f",
        help="Coordenada de longitud de la ubicación del empleo"
    )

st.markdown('</div>', unsafe_allow_html=True)

# ── Botón de predicción ───────────────────────────────────────────────────────
predict_btn = st.button("🔮  Predecir Salario")

if predict_btn:
    if modelo is None:
        st.markdown("""
        <div class="error-box">
            ❌ No se puede predecir porque el modelo no está cargado. 
            Verifica que los archivos exportados de Colab estén en la carpeta correcta.
        </div>
        """, unsafe_allow_html=True)
    else:
        with st.spinner("Calculando predicción..."):
            try:
                ...
                # Badges con el resumen del input
                ...
                st.markdown(f"""
                <div class="error-box">
                    ❌ Ocurrió un error al hacer la predicción: <code>{str(e)}</code>
                </div>
                """, unsafe_allow_html=True)
...
<div style="text-align:center; color:#4a5568; font-size:0.78rem; margin-top:2rem; font-family:'DM Sans',sans-serif;">
    Proyecto Analítica de Datos · Universidad Icesi · 2025<br>
